...
- 终端字体选择器先展示七个常用预设，再展示 Rust `font-kit` 枚举的本机等宽字体和用户导入字体；默认使用 JetBrains Mono，SF Mono / Menlo 与 Consolas 不随应用分发。只有导入文件名与预设名称完全一致时，才可作为标记为“第三方资源”的预设来源，导入后不得自动切换到该预设。
...
- 文件夹上传和下载由 Rust 递归展开为逐文件 capability 并原子加入同一传输队列，目录层级与空目录必须保留；不得跟随符号链接或特殊文件。文件夹根目标已存在时默认拒绝合并或覆盖。
...
- 编辑连接弹窗打开时允许通过专用 command 读取该连接的已保存凭据，以便原生密码输入框按真实长度渲染掩码并支持直接修改；列表加载和其他页面不能预取。秘密只保留在当前表单的短生命周期内，切换认证方式或关闭表单后必须立即从 React 状态清除；小眼睛只切换当前值的明文与掩码显示。
...
- 产品默认采用紧凑生产力布局：以 `4px` 为间距基数，优先使用 `8 / 12 / 16 / 24 / 32px`，不得用大面积留白、超高控件或嵌套大卡片制造层级。
- 常规输入框、分段选择和文字按钮优先使用 `32px` 紧凑规格；完整表单控件最高 `36px`，图标按钮使用 `24–28px`，只有明确的页面主操作可放宽到 `36–40px`。
- `Dialog` 和设置卡片默认使用 `16px` 内容内边距；控件宽度按内容和任务决定，不无条件拉满。相关字段优先双列、组合输入或紧凑横向排列，小窗口再响应式回落。
- 数值、ETA、状态和操作等动态内容必须使用固定宽度轨道；值变化不能推动按钮或改变同组控件宽度。可变文字优先截断次要信息，关键数值和操作始终可见。
- 层级优先通过单像素 `Separator`、语义背景和克制圆角表达；阴影只用于浮层，不为每个字段额外套边框、卡片或说明容器。详细规格以 `docs/ui-design-system.md` 为准。
