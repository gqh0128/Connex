...
预览按连接名称进行不区分大小写的冲突检测，并保留来源文件、行号、解析警告和明确的跳过原因。`Match` 条件块不参与基础导入；使用 `ProxyCommand`、`ProxyJump`、无效端口或未解析动态令牌的候选项必须标记跳过，不能静默降级为直接连接。含 `IdentityFile` 的候选项导入为私钥认证；未提供私钥的候选项导入为密码认证且凭据为空，不读取或推断密码、口令，也不默认依赖本机 SSH Agent。升级 migration 会把旧版本导入为 Agent 的 SSH config 连接同步转换为空密码认证。用户可以逐项选择，并对同名连接统一采用覆盖、跳过或保留两份；保留两份由 service 生成不重复名称。预览返回覆盖全部展开配置的 SHA-256 指纹，提交时重新扫描并校验指纹和当前数据库冲突；新增、覆盖及覆盖时清除旧凭据必须在单个 SQLite 事务中完成，不能产生部分批次。连接配置持久化 `manual` 或 `ssh_config` 来源：普通新建和历史连接默认为手动来源，SSH config 新增或覆盖写入导入来源，普通编辑不改变来源；版本化连接备份携带该字段，并把旧备份缺失来源安全解释为手动添加。
...
上传文件选择由 Rust 通过系统原生多选对话框发起。Rust 立即校验普通文件，记录文件 identity、大小与修改时间，并为每个文件签发不可猜测的 transfer ID；授权同时绑定 SSH session、远程目录、方向和源文件，React 只接收 ID、显示名称与总字节，后续 command 不能把本地路径或远端目标替换成其他值。整批选择必须先通过单个 attach command 原子接管，成功后才加入前端队列；attach 前的孤儿授权使用短 TTL 回收，attach 后不因长时间排队、暂停或重试自动过期。等待任务因此在开始 I/O 前也有总大小，可参与整个队列的 ETA。Rust 使用 `64 KiB` 缓冲区把本地文件直接写入远程目录下带独立 attempt ID 的 Connex 临时文件，通过 Channel 回传字节进度和速度；暂停时关闭当前句柄但保留该 attempt 临时文件，继续时重新验证本地源文件和远端目标，再以远端临时文件的真实长度同时定位本地与远端句柄，从断点续写。成功关闭远程句柄、再次确认源文件 identity、大小、修改时间与 EOF 后再提交为目标文件。默认拒绝覆盖同名文件，取消、失败或会话关闭时终止任务并清理可识别的临时文件。

单文件下载只对远端普通文件开放。每次下载都由 Rust 打开系统保存对话框取得明确的本地目标，不保存固定下载目录；授权绑定 session、选择时的远端普通文件快照、本地父目录 identity 与目标初始状态，并把该快照的权威总字节返回队列。任务同样在 attach 成功后才加入队列。每次尝试开始和结束时都要匹配远端路径、类型、大小与修改时间，排队、暂停或重试期间源文件被替换时必须安全失败。Rust 从 SFTP 句柄分块读取到目标同目录、带独立 attempt ID 的 `.part` 文件；暂停时保留该文件，继续时验证它仍是普通文件、长度没有超过源文件，并把本地与远端句柄定位到该真实长度后继续。完成后确认远端大小、修改时间与 EOF 未变化，并完成远端关闭、本地 `flush` 与 `sync_all` 后才提交。选择时不存在的目标使用同文件系统 hard link 实现原子 no-clobber，若其后出现同名文件则安全失败；选择时已经存在且用户确认覆盖的普通文件必须在提交前再次匹配原 identity，不能先删除原目标。取消、失败或会话关闭只清理该任务可识别的临时文件。

文件夹上传和下载不建立第二套传输协议：Rust 递归扫描并保留相对目录层级，提前创建目标目录树，再为每个普通文件签发独立 capability，并以一个原子批次加入现有 FIFO 队列。空目录在准备阶段直接创建并记录完成状态；普通文件继续复用现有并发、进度、ETA、暂停、断点续传、重试和取消。递归过程不跟随符号链接或特殊文件，发现这类条目会终止准备并给出明确错误。文件夹根目标已存在时拒绝合并或覆盖，单批最多 `1024` 个文件、`4096` 个目录和 `64` 层深度。

传输队列是 `features/transfers` 内的短生命周期交互状态，Rust manager 负责本地路径授权、实际上传/下载、进度、暂停、取消与回收。上传与下载共用一个 FIFO 调度器，暂停与取消使用独立的类型化 command；前端默认全局并发上限为 `3`，Rust 同时执行同样的硬上限，避免绕过界面无限启动 I/O；策略常量集中在 `transferPolicy.ts`。调度 claim 与任务状态由同步权威状态线性化，迟到的 progress、成功、暂停或失败回调必须同时匹配 `running` 状态、当前 `attempt` 与单调递增的 run generation，不能覆盖取消或下一次手动重试。已暂停任务释放并发槽但继续持有绑定两端目标的 capability 和当前 attempt ID；活动、等待、暂停及仍持有可重试授权的失败任务永不因历史上限被裁掉，其他完成、取消与失败历史最多保留最近 `100` 项。

瞬时的上传、下载或 SFTP 错误最多自动重试 `2` 次，退避为 `1s / 3s`；路径无效、同名冲突、本地文件不可用、断点数据失效、会话已结束、用户暂停和用户取消不自动重试。暂停/继续属于同一次 attempt，不增加 attempt 计数；普通重试保留逻辑任务 ID 与原路径授权，但从零开始并使用新的 attempt ID 创建临时文件。最终失败且授权仍有效的任务允许用户手动重试。未 attach 的孤儿授权会过期；已 attach 授权只在成功、取消、不可恢复失败、用户放弃重试或 session 关闭时由 Rust 吊销，不能因队列等待或暂停时间过长而失效。

暂停、取消与最终提交通过 Rust 原子 lifecycle 竞争：进入提交阶段前可接受暂停或取消，取消可以覆盖正在收口的暂停；进入提交阶段后两者都明确返回 `tooLate`。界面只有在运行 command 以 `transfer_paused` 收口后才把任务归入已暂停，并且不能在 Rust 确认 `accepted` 前承诺已经取消。排队授权的释放请求待确认期间暂停调度；IPC 释放失败时保留“等待释放”状态和再次释放入口，不能把任务静默落为已取消。关闭或重连单个 session 时，前端先把归属该 session 的等待、运行、重试、暂停及保留授权失败任务统一收口为已取消，Rust session close 再作为权威路径撤销 capability、终止传输并限时等待临时文件清理；窗口或应用关闭执行同样的后端清理。超时后继续退出，避免应用无限卡住。

Rust 约每 `200ms` 采样吞吐并用 EMA 平滑后发送进度。单文件 ETA 使用剩余字节除以当前平滑速度；整个队列 ETA 以当前运行任务的剩余时间作为三个并发槽的初始负载，再按 FIFO 把等待任务分配给最早空闲槽。总大小或稳定速度不足时返回未知，界面显示“估算中”，不能伪造精确时间。
...
创建和编辑连接时允许密码认证暂不保存密码；私钥认证保存私钥路径，并按需保存私钥口令。编辑已有凭据时字段留空表示保留原值，切换认证方式时清理不再适用的秘密；删除连接时由服务层显式移除密文。连接元数据和密文虽位于同一数据库，但当前通过独立异步仓库操作，服务层继续采用“先写凭据、再写元数据、失败时恢复”的补偿顺序，不能依赖跨调用事务。旧版本按连接 UUID 保存到系统凭据管理器的条目采用按需迁移：第一次连接、显示或导出时读取旧秘密、写入 SQLite 密文，成功后删除旧条目，避免应用启动时批量触发系统授权。

启动 SSH 会话时，前端只提交连接 ID 和终端尺寸。Rust 侧读取 SQLite 配置和加密凭据，使用进程内本机主密钥解密后直接构造认证请求；普通连接流程中密码和私钥口令不返回 React，也不进入会话 IPC 输入。密码认证没有已保存密码时直接返回“未输入密码”的可操作错误，用户编辑连接补录后即可恢复双击直连。

编辑连接提供唯一例外：弹窗打开后，前端通过专用 `reveal_connection_credential` command 读取当前连接的秘密，并只存入该表单的短生命周期 state，让原生 `password` 输入框生成与真实长度一致的掩码并支持直接编辑；列表加载和其他页面不能预取。前端单独记录凭据是否被修改，未改动时更新连接不回传秘密，主动删空时显式要求 Rust 删除旧凭据；连接测试同样不能在显式清空后回退读取旧值。认证方式变化、表单关闭或组件卸载后立即清除前端临时解密值。该能力不改变凭据的加密存储位置。
