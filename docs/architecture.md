...
上传文件选择由 Rust 通过系统原生多选对话框发起。Rust 立即校验普通文件，记录文件 identity、大小与修改时间，并为每个文件签发不可猜测的 transfer ID；授权同时绑定 SSH session、远程目录、方向和源文件，React 只接收 ID、显示名称与总字节，后续 command 不能把本地路径或远端目标替换成其他值。整批选择必须先通过单个 attach command 原子接管，成功后才加入前端队列；attach 前的孤儿授权使用短 TTL 回收，attach 后不因长时间排队或重试自动过期。等待任务因此在开始 I/O 前也有总大小，可参与整个队列的 ETA。Rust 使用 `64 KiB` 缓冲区把本地文件直接写入远程目录下带独立 attempt ID 的 Connex 临时文件，通过 Channel 回传字节进度和速度；成功关闭远程句柄、再次确认源文件 identity、大小、修改时间与 EOF 后再提交为目标文件。默认拒绝覆盖同名文件，取消、失败或会话关闭时终止任务并清理可识别的临时文件。

下载只对远端普通文件开放。每次下载都由 Rust 打开系统保存对话框取得明确的本地目标，不保存固定下载目录；授权绑定 session、选择时的远端普通文件快照、本地父目录 identity 与目标初始状态，并把该快照的权威总字节返回队列。任务同样在 attach 成功后才加入队列。每次尝试开始和结束时都要匹配远端路径、类型、大小与修改时间，排队或重试期间源文件被替换时必须安全失败。Rust 从 SFTP 句柄分块读取到目标同目录、带独立 attempt ID 的 `.part` 文件，确认远端大小、修改时间与 EOF 未变化，并完成远端关闭、本地 `flush` 与 `sync_all` 后才提交。选择时不存在的目标使用同文件系统 hard link 实现原子 no-clobber，若其后出现同名文件则安全失败；选择时已经存在且用户确认覆盖的普通文件必须在提交前再次匹配原 identity，不能先删除原目标。取消或失败只清理该任务可识别的临时文件。

传输队列是 `features/transfers` 内的短生命周期交互状态，Rust manager 负责本地路径授权、实际上传/下载、进度、取消与回收。上传与下载共用一个 FIFO 调度器和取消 command，前端默认全局并发上限为 `3`，Rust 同时执行同样的硬上限，避免绕过界面无限启动 I/O；策略常量集中在 `transferPolicy.ts`。调度 claim 与任务状态由同步权威状态线性化，迟到的 progress、成功或失败回调必须同时匹配 `running` 状态、当前 `attempt` 与单调递增的 run generation，不能覆盖取消或下一次手动重试。活动、等待及仍持有可重试授权的失败任务永不因历史上限被裁掉，其他完成、取消与失败历史最多保留最近 `100` 项。

瞬时的上传、下载或 SFTP 错误最多自动重试 `2` 次，退避为 `1s / 3s`；路径无效、同名冲突、本地文件不可用、会话已结束和用户取消不自动重试。重试保留逻辑任务 ID 与原路径授权，但每次从零开始并使用新的 attempt ID 创建临时文件；最终失败且授权仍有效的任务允许用户手动重试。未 attach 的孤儿授权会过期；已 attach 授权只在成功、取消、不可恢复失败、用户放弃重试或 session 关闭时由 Rust 吊销，不能因队列等待时间过长而失效。

取消与最终提交通过 Rust 原子 lifecycle 竞争：进入提交阶段前接受取消，进入后明确返回 `tooLate`，界面不能在 Rust 确认 `accepted` 前承诺已经取消。排队授权的释放请求待确认期间暂停调度；IPC 释放失败时保留“等待释放”状态和再次释放入口，不能把任务静默落为已取消。关闭或重连单个 session 时，前端先把归属该 session 的等待、运行、重试及保留授权失败任务统一收口为已取消，Rust session close 再作为权威路径撤销 capability、终止传输并限时等待临时文件清理；窗口或应用关闭执行同样的后端清理。超时后继续退出，避免应用无限卡住。
