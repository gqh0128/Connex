...
- 第一阶段保留系统原生标题栏，不实现自定义窗口标题栏和窗口按钮。

当前暗色应用壳的三栏方向继续沿用，但它只是起点；本文中的浅色主题、自适应和传输区域规则才是完整目标。

## 2. 信息架构与主布局

应用固定由连接导航、会话工作区、远程文件、传输抽屉和状态栏组成：

```text
┌──────── 连接侧栏 ────────┬──────────── 会话标签栏 ────────────┬── 文件操作 ──┐
│ 品牌 / 搜索 / 新建       │ 活动会话、状态、关闭和新建          │ SFTP 工具栏   │
│                          ├─────────────────────────────────────┼──────────────┤
│ 最近使用 / 分组 / 连接   │                                     │ 路径与文件表格 │
│                          │              终端                   │              │
│                          │                                     │              │
...
| 传输抽屉     |      `176px` | 可调整 `120px–40vh`，默认收起且不因新任务强制展开  |
| 状态栏       |       `24px` | 固定高度，跨窗口底部                               |
...
- 传输队列是工作区底部抽屉，横跨终端和文件区，显示方向、文件名、进度、速度、剩余时间和操作。
- 新传输只更新状态栏摘要并使用轻量通知，不自动展开抽屉打断终端输入。失败任务应明显提示并提供重试。
...
| 传输任务         | `Progress`、`Badge`、`Collapsible`                          |
